...
static void WriteDefaults(const char* path){
  ...
  fputs(
    ...
    "ReflexSleep=1\n"
    "; Feed the DLSS-SR output as the HUD-less color buffer (0 = backbuffer only, like PureDark's mod).\n"
    "; The SR output is pre-post-processing; if it does not match the backbuffer's color space it causes halos/ghosting.\n"
    "TagHUDLess=0\n"
